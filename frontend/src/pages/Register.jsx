...
const Register = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.name || !formData.email || !formData.password) {
      alert("Mohon lengkapi data esensial Anda!");
      return;
    }
    navigate('/pre-test'); 
  };
  ...
  return (
    <div style={{ display: 'flex', backgroundColor: '#f4f6f9', minHeight: '100vh', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      
      {/* ================= SISI KIRI: BRANDING AREA (FULL BACKGROUND) ================= */}
      <div style={{ 
        flex: '1.1', // Sedikit lebih lebar untuk menyeimbangkan visual gambar
        backgroundImage: "url('https://images.unsplash.com/photo-1531403009284-440f080d1e12?auto=format&fit=crop&w=1200&q=80')", // Ganti dengan path lokal assets Anda nanti
       ...
       {/* LOGO AMATI */}
        ...
        <span style={{ fontWeight: '800', fontSize: '1.6rem', color: '#fff', letterSpacing: '0.8px', textShadow: '0 2px 5px rgba(0,0,0,0.25)' }}>
            AMATI
        </span>
        </div>

        {/* Teks atau Kredit kecil di bawah jika diperlukan (Tetap biarkan sama di bawahnya) */}
        <div style={{ zIndex: 2, color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', textAlign: 'left', marginTop: '10px' }}>
      ...
      <div style={{ flex: '1', backgroundColor: '#f4f6f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '25px', textAlign: 'left' }}>
            <h1 style={{ margin: 0, color: '#002d72', fontSize: '2.2rem', fontWeight: '800', letterSpacing: '-0.5px' }}>Sign up</h1>
            <p style={{ margin: '8px 0 0 0', color: '#666', fontSize: '0.9rem' }}>
              Already have an account? <span onClick={() => navigate('/login')} style={{ color: '#007bff', fontWeight: '700', cursor: 'pointer', textDecoration: 'none' }}>Sign in here</span>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ textAlign: 'left' }}>
              <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.85rem', fontWeight: '700', color: '#333' }}>Name</label>
              <input 
                type="text" 
                name="name"
                value={formData.name}
                ...
                style={{ width: '100%', padding: '14px 18px', borderRadius: '12px', border: '1px solid #ddd', backgroundColor: '#fff', outline: 'none', fontSize: '0.9rem', color: '#333' }} 
              />
            </div>

            {/* INPUT 2: BIRTH DATE */}
            <div style={{ textAlign: 'left' }}>
              <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.85rem', fontWeight: '700', color: '#333' }}>Birth Date</label>
              <input 
                type="text" 
                name="birthDate"
            ...
            <div style={{ textAlign: 'left' }}>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <input 
                  ...
                  style={{ width: '100%', padding: '14px 50px 14px 18px', borderRadius: '12px', border: '1px solid #ddd', backgroundColor: '#fff', outline: 'none', fontSize: '0.9rem', color: '#333' }} 
                />
                <span 
                  onClick={() => setShowPassword(!showPassword)} 
                  style={{ position: 'absolute', right: '18px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                >
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginTop: '4px', textAlign: 'left' }}>
              <input type="checkbox" required style={{ marginTop: '3px', cursor: 'pointer' }} />
              <span style={{ fontSize: '0.8rem', color: '#666', lineHeight: '1.4' }}>
                By Clicking create an account, I agree that I have read and accepted the <span style={{ color: '#007bff', cursor: 'pointer', fontWeight: '600' }}>term of use</span> and <span style={{ color: '#007bff', cursor: 'pointer', fontWeight: '600' }}>privacy policy</span>.
              </span>
            </div>
            ...
            <button 
              type="submit"
              style={{ width: '100%', padding: '14px', backgroundColor: '#3b71ca', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: '0.95rem', cursor: 'pointer', marginTop: '10px', boxShadow: '0 4px 15px rgba(59, 113, 202, 0.2)' }}
            >
              Create an Account
            </button>
          </form>
